...
# Returns a number that is a rounded
def rndint(x):
    return int(round(x))


# Check if x is smaller or larger than 2 numbers
...
# Inner class which represents the games Paddles
class Paddle(object):
...
# A class which represents the ball
class Ball(object):
...
# Check for the player's input - if Quit / Escape were pressed - return False
# If another key was pressed - update player's paddels key info
def get_input():
...
# Call the balls position update
def update():
    for ball in balls:
        ball.update()
...
def draw():
    ...
    if between_rounds_timer > 0:
        ...
        count = pygame.transform.smoothscale(count,
                                             list(map(rndint, [count.get_width() * sc, count.get_height() * sc])))
